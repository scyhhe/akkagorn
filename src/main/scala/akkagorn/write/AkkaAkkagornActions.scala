...
class AkkaAkkagornActions(behavior: AkkagornBehaviorRoot)(implicit
  ...
  def registerSubscriber(
      cmd: ActorRef[
        Either[RegisterSubscriberFailure, Unit]
      ] => RegisterSubscriber
  ): Future[Either[RegisterSubscriberFailure, Unit]] = ???
  def publishMessage(
      cmd: ActorRef[Either[PublishMessageFailure, Unit]] => PublishMessage
  ): Future[Either[PublishMessageFailure, Unit]] = ???
}
